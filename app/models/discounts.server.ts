...
export async function createCodeDiscount(
        ...
        metafields: [
          {
            ...
            value: JSON.stringify({
              ...
              minimumQuantity: configuration.minimumQuantity || 0,
              quantityToDiscount: configuration.quantityToDiscount || 0,
            }),
          },
        ],
      },
  ...
  if (errors) {
      return { errors: errors.map((e: any) => ({ message: e.message, field: [] })) };
  }
...
export async function getDiscount(request: Request, id: string) {
  ...
  const configurationField = responseJson.data.discountNode.configurationField;
  const configuration = configurationField ? JSON.parse(
  ...
  return {
    discount: {
      ...
      configuration: {
        ...configuration,
        metafieldId: configurationField?.id,
      },
    },
  };
}
